...
#define S_FUNCTION_NAME carboncombiner_asm2d
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    ssSetNumInputs(        S, 27);   /* number of inputs                     (1 Qcarb + 20 ASM2d states + 21 Q + T + 5 dummy states)*/
    ssSetNumOutputs(       S, 26);   /* number of outputs                    (19 ASM2d states +  20 Q + T + 5 dummy states)*/
    ssSetDirectFeedThrough(S, 1);   /* direct feedthrough flag               */
    ssSetNumSampleTimes(   S, 1);   /* number of sample times                */
    ssSetNumSFcnParams(    S, 1);   /* number of input arguments             (CARBONSOURCECONC )*/
...
static void mdlOutputs(double *y, double *x, double *u, SimStruct *S, int tid)
{
  ...
  if ((u[0] > 0) || (u[20] > 0)) {
    ...
    y[2]=(u[3]*u[20] + csourceconc*u[0])/(u[0]+u[20]);
