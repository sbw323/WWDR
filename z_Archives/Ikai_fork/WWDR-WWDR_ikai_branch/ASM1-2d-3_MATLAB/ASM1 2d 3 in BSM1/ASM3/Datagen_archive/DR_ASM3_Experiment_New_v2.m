...
if ~ss_done
    ...
    stateset;
    ss_done = true;
    disp('Phase 2 steady state completed.');
end
...
% === PHASE 3: Main Data Generation Loop ===
...
for segment = 1:total_segments
    ...
    current_start_time = cal_time + (segment - 1) * pause_time;
    current_end_time = cal_time + segment * pause_time;
    
    disp(['=== Processing Segment ', num2str(segment), ' (t = ', num2str(current_start_time), ' to ', num2str(current_end_time), ' days) ===']);
    ...
    set_param(model, 'StartTime', num2str(cal_time + (segment - 1) * pause_time));
    set_param(model, 'StopTime', num2str(cal_time + segment * pause_time));
    safe_set_outputtimes(model);
    ...
    disp(['Experimental data saved for segment ', num2str(segment)]);
    ...
    disp(['Nominal data saved for segment ', num2str(segment)]);
    ...
    disp(['Segment ', num2str(segment), ' completed successfully.']);
end
