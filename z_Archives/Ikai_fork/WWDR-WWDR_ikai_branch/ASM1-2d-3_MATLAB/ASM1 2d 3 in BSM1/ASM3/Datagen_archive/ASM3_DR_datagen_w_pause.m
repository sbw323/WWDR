...
cal_time = 69;                     % Calibration time [days]
...
for current_segment = 1:total_segments
    ...
    % ADD: iteration variable for data writers
    PROTECTED_VARS.iteration = current_segment;
    ...
    iteration = PROTECTED_VARS.iteration;  % CRITICAL: Restore iteration for data writers
    
    % ADDITIONAL: Create backup variables in case data writers expect different names
    iter = PROTECTED_VARS.iteration;
    segment_iter = PROTECTED_VARS.iteration;
    current_iteration = PROTECTED_VARS.iteration;
    
    fprintf('  Iteration/Segment: %d\n', iteration);
    ...
    % ENSURE iteration variable is still available before calling data writers
    if ~exist('iteration', 'var')
        iteration = PROTECTED_VARS.iteration;
        fprintf('  WARNING: iteration variable was cleared, restored to %d\n', iteration);
    end
    
    % Save experimental data (with error handling and iteration verification)
    try
        fprintf('  Calling data writers with iteration = %d\n', iteration);
        
        if exist('Data_writer_newer', 'file')
            Data_writer_newer;
            fprintf('    ✓ Data_writer_newer completed\n');
        end
        if exist('Data_writer_reac', 'file')
            Data_writer_reac;
            fprintf('    ✓ Data_writer_reac completed\n');
        end
        if exist('perf_plant_LT_DR', 'file')
            perf_plant_LT_DR;
            fprintf('    ✓ perf_plant_LT_DR completed\n');
        end
        if exist('figure_writer', 'file')
            figure_writer;
            fprintf('    ✓ figure_writer completed\n');
        end
        fprintf('    ✓ Experimental data saved for iteration %d\n', iteration);
    catch ME
        fprintf('    ⚠ Warning: Some data writers failed for iteration %d: %s\n', iteration, ME.message);
    ...
    % PRESERVE iteration in the saved workspace for debugging
    iteration_backup = PROTECTED_VARS.iteration;
    save('workspace_experimental_calibrated.mat');
    
    exp_time = toc(exp_start);
    fprintf('  ✓ Experimental segment %d completed in %.1f seconds\n', iteration, exp_time);
    ...
    % Optional: Save detailed progress with iteration info
    progress_info = struct();
    progress_info.completed_segments = current_segment;
    progress_info.total_segments = total_segments;
    progress_info.last_iteration = PROTECTED_VARS.iteration;
    progress_info.timestamp = datetime('now');
    save('detailed_progress.mat', 'progress_info');
end
...
fprintf('  Final iteration number: %d\n', total_segments);
