...
% origin (filamentous bulking, foaming and rising sludge), last version
% March 2009
% Copyright: Dr Joaquim Comas and Dr Ignasi Rodriguez-Roda, LEQUIA, Girona
% University, Spain
...
CODevec=settlerpart(:,37).*CODe; % settlerpart(:,37) effluent flow rate (changed in BSM1*)
...
Qwasteflow = settlerpart(:,22);
TSSwasteconc = settlerpart(:,53);%%(changed from BSM1*)
TSSuvec2 =TSSwasteconc.*Qwasteflow; % TSS in WAS, in g/d
TSSevec2=settlerpart(:,36).*settlerpart(:,37); %TSS in the effluent, in g/d (changed from BSM1*)
...
NDefBulking1_raw = NDefBulking1;
NDefBulking1 = smoothing_data(NDefBulking1,3)';
...
disp('Qualitative risk criteria for settling problems')
disp('-----------------------------------------------')
disp(' ')
...
LowDOBulking1_raw = LowDOBulking1;
LowDOBulking1 = smoothing_data(LowDOBulking1,3)';
...
LowFtoMBulking1=evalfis([reac1part(:,2) SRTvec], bulkingfis); %react(:,2)=reactor SS
...
%To evaluate the output of the bulkingfis fuzzy system (low F/M bulking) for a given input; inpart (:,2)=influent SS
LowFtoMBulking2=evalfis([FtoM_2vec SRTvec], bulkingfis);%F/M_2 food to microorganisms ratio
...
% Temperature factor correction of risk due to Microthrix parvicella

Factor=1.2*exp(-((reac5part(:,16)-5).^2/625));
LowFtoMBulking=LowFtoMBulking.*Factor;

for i=1:length(LowFtoMBulking);
if LowFtoMBulking(i)>1;
    LowFtoMBulking(i)=1;
else
    LowFtoMBulking(i)=LowFtoMBulking(i);
end
end

LowFtoMBulking_raw =LowFtoMBulking;

LowFtoMBulking = smoothing_data(LowFtoMBulking,3)';
...
LowFtoMFoaming2=evalfis([FtoM_2vec SRTvec], foamingfis);
    
%%%% Temperature factor correction of risk due to Microthrix parvicella

Factor=1.2*exp(-((reac5part(:,16)-5).^2/625));
LowFtoMFoaming2=LowFtoMFoaming2.*Factor;  

for i=1:length(LowFtoMFoaming2);
if LowFtoMFoaming2(i)>1;
    LowFtoMFoaming2(i)=1;
else
    LowFtoMFoaming2(i)=LowFtoMFoaming2(i);
end
end

LowFtoMFoaming2_raw = LowFtoMFoaming2;

LowFtoMFoaming2 = smoothing_data(LowFtoMFoaming2,3)';
...
HighRBOMfractionFoaming1=evalfis([FtoM_2vec SsXsratio], foamingfis);
...
%To evaluate the output of the foamingfis fuzzy system (due to high readily biodegradable organic matter fraction) for a given input, in another way
HighRBOMfractionFoaming2=evalfis([SRTvec SsXsratio], foamingfis);% SS/XS ratio
...
HighRBOMfractionFoaming_raw= HighRBOMfractionFoaming; 

HighRBOMfractionFoaming=smoothing_data(HighRBOMfractionFoaming,3)';
...
SludgeVolumeInClarifier=(settlerpart(:,44).*600+settlerpart(:,45).*600+settlerpart(:,46).*600+settlerpart(:,47).*600+settlerpart(:,48).*600+settlerpart(:,49).*600+settlerpart(:,50).*600+settlerpart(:,51).*600+settlerpart(:,52).*600+settlerpart(:,53).*600)./settlerpart(:,53);

nitrifiers_fraction=1;

SNOHighLimit= 11.003972.*(exp(-0.020295*reac5part(:,16)));
mu_H= 4*exp((log(4/3)/5)*(reac5part(:,16)- 15));
...
for i=1:length(SNOHighLimit)      
Risingfis = newfis('Rising');
%%%definition of the inputs
Risingfis = addvar(Risingfis,'input','Sno',[0 40]);
Risingfis = addmf(Risingfis,'input',1,'L','trapmf',[-1.429 -1.429 2 5]);
Risingfis = addmf(Risingfis,'input',1,'N','trimf',[2 5 8]);
Risingfis = addmf(Risingfis,'input',1,'H','trapmf',[5 8 40.27 41.87]);
Risingfis = addvar(Risingfis,'input','ratiodn',[0 2.2]);
Risingfis = addmf(Risingfis,'input',2,'L','trapmf',[-0.135 -0.0437 Limit1(i) Limit2(i)]);
Risingfis = addmf(Risingfis,'input',2,'N','trimf',[Limit1(i) Limit2(i) Limit3(i)]);
Risingfis = addmf(Risingfis,'input',2,'H','trapmf',[Limit2(i) Limit3(i) 2.205 2.272]);
%%%definition of the outputs
Risingfis = addvar(Risingfis, 'output','Rising',[-0.2 1.2]);
Risingfis = addmf(Risingfis,'output',1,'Low','trimf',[-0.2 0 0.2]);
Risingfis = addmf(Risingfis,'output',1,'Medium','trimf',[0.2 0.5 0.8]);
Risingfis = addmf(Risingfis,'output',1,'High','trimf',[0.8 1 1.2]);
...
Rising1(i)=evalfis([SNOOutReac5(i) ((SNOHighLimit(i)./Rdn2(i))+t_delay(i))], Risingfis);
end

Rising1_raw = Rising1;
Rising1=smoothing_data(Rising1,0.0833)';
...
% plotting Fuzzy Settling problems conditions
   figure(17)
   plot (time_eval(1:(end-1)),LowDOBulking1_raw,'g')
   hold on
   plot (time_eval(1:(end-1)),LowDOBulking1,'b')
   xlabel ('time (days)')
   ylim([0 1])
   ...
   figure(18)
   plot (time_eval(1:(end-1)),NDefBulking1_raw, 'g')
   hold on
   plot (time_eval(1:(end-1)),NDefBulking1,'b')
   xlabel ('time (days)')
   ylim([0 1])
   ...
   figure(19)
   plot (time_eval(1:(end-1)),LowFtoMBulking_raw,'g')
   hold on
   plot (time_eval(1:(end-1)),LowFtoMBulking,'b')
   xlabel ('time (days)')
   ylim([0 1])
   ...
   figure(20)
   plot (time_eval(1:(end-1)),LowFtoMFoaming2_raw,'g')
   hold on
   plot (time_eval(1:(end-1)),LowFtoMFoaming2,'b')
   xlabel ('time (days)')
   ylim([0 1])
   ...
   figure(21)
   plot (time_eval(1:(end-1)),HighRBOMfractionFoaming_raw,'g')
   hold on
   plot (time_eval(1:(end-1)),HighRBOMfractionFoaming,'b')
   xlabel ('time (days)')
   ylim([0 1])
...
if not(isempty(NofRising1))
   figure(22)
   plot (time_eval(1:(end-1)),Rising1_raw,'g')
   hold on
   plot (time_eval(1:(end-1)),Rising1,'b')
   xlabel ('time (days)')
   ylim([0 1])
